...
# re 623 top 5
CONFIG_DIR = os.path.join("training_configs", "_re_623_top_5")

# re 623 top 5 dev
# CONFIG_DIR = os.path.join("training_configs", "_re_623_top_5_dev")
...
def load_and_combine_metadata_with_ner_results(ner_results_path, test_data_path):
	...
	save_json_data(combined_data, os.path.join("combined_data", "combined_ner_and_test_data_6.json"))
	...
	for filename in os.listdir(CONFIG_DIR):
		...
		if os.path.isfile(file_path):
			...
			re_inference = REInference(
				test_data_path=os.path.join("combined_data", "combined_ner_and_test_data_6.json"),
				...
				save_path=os.path.join(
					"data_inference_results_re_evaluated_on_test_ee9",
					f"{config['subtask']}",
